...
class InsPartnerLedger(models.TransientModel):
    ...
    def prepare_where(self, mode='strict'):
        ...
        where += ' AND l.company_id in %s ' % str(
            tuple(cmpny_ids) + tuple([0]))

        if self.journal_ids:
            where += ' AND j.id IN %s ' % str(
                tuple(self.journal_ids.ids) + tuple([0]))
        if self.account_ids:
            where += ' AND a.id IN %s ' % str(
                tuple(self.account_ids.ids) + tuple([0]))
        # if self.company_id:
    ...
    def prepare_main_lines(self):
        ...
        partner_ids = self.env['res.partner'].search(
            partner_company_domain, order='name asc')
        pl_lines = []
        for partner in partner_ids:
            ...
            if self.display_accounts == 'with_transaction' and (
                    self.currency_id.is_zero(result.get(
                        'debit')) and self.currency_id.is_zero(result.get('credit'))
            ):
                continue
    ...
    def prepare_values_for_component(self):
        ...
        model_id = self.env['ir.model'].sudo().search(
            [('model', '=', 'ins.partner.ledger')], limit=1)

        date_range = {'choices': [], 'selectedValue': {
            'value': self.date_range}}
        target_moves = {'choices': [], 'selectedValue': {
            'value': self.target_moves}}
        display_accounts = {'choices': [], 'selectedValue': {
            'value': self.display_accounts}}
        include_initial_balance = {'choices': [], 'selectedValue': {
            'value': self.include_initial_balance}}
        reconciled = {'choices': [], 'selectedValue': {
            'value': self.reconciled}}
        account_type = {'choices': [], 'selectedValue': {
            'value': self.account_type}}

        for field in model_id.field_id:
            if field.name == 'date_range':
                date_range['choices'] = [
                    {'value': a.value, 'label': a.name} for a in field.selection_ids]
            if field.name == 'target_moves':
                target_moves['choices'] = [
                    {'value': a.value, 'label': a.name} for a in field.selection_ids]
            if field.name == 'display_accounts':
                display_accounts['choices'] = [
                    {'value': a.value, 'label': a.name} for a in field.selection_ids]
        ...
        return {
            ...
            'defaultJournalValues': [{'value': a.id, 'label': a.name, 'code': a.code_store} for a in self.journal_ids],
            'defaultAccountTagValues': [{'value': a.id, 'label': a.name, 'code': a.code_store} for a in self.account_tag_ids],
        ...
        for field in model_id.field_id:
            if field.name in vals.keys():
                if field.ttype == 'many2many':
                    if vals[field.name]:
                        update_dict.update(
                            {field.name: [int(a['value']) for a in vals[field.name]]})
                    else:
                        update_dict.update({field.name: [(5,)]})
                if field.ttype == 'selection':
                    update_dict.update(
                        {field.name: vals[field.name]['selectedValue']['value']})
                if field.ttype == 'date':
    ...
    def action_xlsx(self):
        ...
        # Get record and data
        record = self.env['ins.partner.ledger'].browse(
        ...
        row_pos_2 = 0
        row_pos = 0
        sheet.merge_range(0, 0, 0, 8, 'Partner Ledger' +
        ...
        if account_lines:
            for line in account_lines:
                row_pos += 1
                sheet.merge_range(row_pos, 0, row_pos, 5, line.get(
                    'partner_name'), line_header_left)
                ...
                sheet.write(row_pos, 9, float(
                    line.get('balance')), line_header)

                sub_lines = record.prepare_detailed_lines(
                    line.get('id_list'), line.get('partner_id'))

                for sub_line in sub_lines:
                    if sub_line.get('ttype') == 'initial':
                        row_pos += 1
                        sheet.write(row_pos, 5, 'Initial Balance',
                                    line_header_light_ending_bold)
                        sheet.write(row_pos, 6, float(sub_line.get(
                            'debit')), line_header_light_initial)
                        sheet.write(row_pos, 7, float(sub_line.get(
                            'credit')), line_header_light_initial)
                        sheet.write(row_pos, 8, '', line_header_light_initial)
                        sheet.write(row_pos, 9, float(sub_line.get(
                            'balance')), line_header_light_initial)
                    elif sub_line.get('ttype') == 'strict':
                        row_pos += 1
                        sheet.write(row_pos, 0, sub_line.get(
                            'move_name'), line_header_light)
                        sheet.write(row_pos, 1, sub_line.get(
                            'lcode'), line_header_light)
                        datestring = fields.Date.from_string(
                            str(sub_line.get('ldate'))).strftime(lang_id.date_format)
                        sheet.write(row_pos, 2, datestring,
                                    line_header_light_date)
                        if sub_line.get('date_maturity'):
                            datestring = fields.Date.from_string(
                                str(sub_line.get('date_maturity'))).strftime(lang_id.date_format)
                            sheet.write(row_pos, 3, datestring,
                                        line_header_light_date)
                        else:
                            sheet.write(row_pos, 3, datestring,
                                        line_header_light_date)
                        sheet.write(row_pos, 4, sub_line.get(
                            'account_name'), line_header_light)
                        sheet.write(row_pos, 5, sub_line.get(
                            'matching_number') or '', line_header_light)
                        sheet.write(row_pos, 6, float(
                            sub_line.get('debit')), line_header_light)
                        sheet.write(row_pos, 7, float(
                            sub_line.get('credit')), line_header_light)
                        sheet.write(row_pos, 8, float(sub_line.get(
                            'amount_currency')), line_header_light)
                        sheet.write(row_pos, 9, float(
                            sub_line.get('balance')), line_header_light)
                    else:  # Ending Balance
                        row_pos += 1
                        sheet.write(row_pos, 5, 'Ending Balance',
                                    line_header_light_ending_bold)
                        sheet.write(row_pos, 6, float(
                            sub_line.get('debit')), line_header_light_ending)
                        sheet.write(row_pos, 7, float(sub_line.get(
                            'credit')), line_header_light_ending)
                        sheet.write(row_pos, 8, '', line_header_light_ending)
                        sheet.write(row_pos, 9, float(sub_line.get(
                            'balance')), line_header_light_ending)
        ...
        report_id = self.env['common.xlsx.out'].sudo().create(
            {'filedata': result, 'filename': 'PL.xls'})
        return {
