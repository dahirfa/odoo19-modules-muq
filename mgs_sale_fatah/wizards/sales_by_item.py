...
class SalesbyItemDetail(models.TransientModel):
    ...
    def confirm(self):
        ...
        return self.env.ref('mgs_sale_fatah.action_sales_by_item').report_action(self, data=data)

    def export_to_excel(self):
        sales_by_customer_report_obj = self.env['report.mgs_sale_fatah.sales_by_item_report']
...
class SalesbyItemDetailReport(models.AbstractModel):
    _name = 'report.mgs_sale_fatah.sales_by_item_report'
    _description = 'Sales by Item Report'

    
    @api.model
    def _lines(self, date_from, date_to, company_id, user_id, product_ids, team_id, parent_categ_id, categ_id):
        ...
        if parent_categ_id:
            _where_sale += " and pc2.id = %s" % parent_categ_id

        if categ_id:
            _where_sale += " and pc.id = %s" % categ_id
