...
class ReceiptAndPaymentReport(models.AbstractModel):
    ...
    def _lines(self, date_from, date_to, company_id, partner_id, journal_id, user_id, payment_type):
        ...
        rp.phone as phone, aml.date as date, sum(aml.credit-aml.debit) as amount_paid, aj.name['en_US'] as journal_name
        ...
        query += """
        group by am.name, rp.id, rp.name, aml.name, rp.phone, aml.date, aj.name
        order by aml.date"""
