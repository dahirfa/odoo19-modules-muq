...
class InsAnalyticReport(models.TransientModel):
    ...
    def prepare_where(self):
        ...
        where += ' AND anl_line.company_id in %s ' % str(
        ...
        if self.partner_ids:
            where += ' AND p.id IN %s ' % str(
                tuple(self.partner_ids.ids) + tuple([0]))
        if self.account_ids:
        ...
        where += ''' AND anl_line.date >= '%s' AND anl_line.date <= '%s' ''' % (
    ...
    def prepare_main_lines(self):
        ...
        analytic_ids = self.env['account.analytic.account'].search(
            analytic_company_domain, order='code asc')
        analytic_lines = []
    ...
    def prepare_detailed_lines(self, move_line_ids=[], analytic_account_id=False):
        ...
        # Current
        where_current = self.prepare_where(
        ) + ' AND anl_line.id in %s ' % str(tuple(move_line_ids) + tuple([0]))
        sql = ('''
                SELECT
                    anl_line.id AS id,
    ...
    def prepare_values_for_component(self):
        ...
        model_id = self.env['ir.model'].sudo().search(
            [('model', '=', 'ins.analytic.report')], limit=1)
    ...
    def action_xlsx(self):
        ...
        # Get record and data
        record = self.env['ins.analytic.report'].browse(
        ...
        row_pos_2 = 0
        row_pos = 0
        sheet.merge_range(0, 0, 0, 8, 'Analytic Report' +
        ...
        if account_lines:
            for line in account_lines:
                ...
                sheet.merge_range(row_pos, 0, row_pos, 5,
                                  '            ' +
                                  line.get('analytic_code', '') + ' - ' +
                                  line.get('analytic_name', ''),
                                  line_header_left)
                sheet.write(row_pos, 6, float(line.get('amount')), line_header)

                sub_lines = record.prepare_detailed_lines(
                    line.get('id_list'), line.get('account_id'))
                if record.include_details == 'yes':
                    for sub_line in sub_lines:
                        row_pos += 1
                        datestring = fields.Date.from_string(
                            str(sub_line.get('date'))).strftime(lang_id.date_format)
                        sheet.write(row_pos, 0, datestring,
                                    line_header_light_date)
                        sheet.write(row_pos, 1, sub_line.get(
                            'partner_name') or '', line_header_light)
                        sheet.write(row_pos, 2, sub_line.get(
                            'journal_code') or '', line_header_light)
                        sheet.write(row_pos, 3, sub_line.get(
                            'account_name') or '', line_header_light)
                        sheet.write(row_pos, 4, sub_line.get(
                            'plan') or '', line_header_light)
                        sheet.write(row_pos, 5, sub_line.get(
                            'product') or '', line_header_light)
                        sheet.write(row_pos, 6, float(
                            sub_line.get('amount')), line_header_light)
        ...
        report_id = self.env['common.xlsx.out'].sudo().create(
            {'filedata': result, 'filename': 'GL.xls'})
        return {
