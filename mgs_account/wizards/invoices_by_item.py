...
class InvoicesbyItem(models.TransientModel):
    ...
    def export_to_excel(self):
        ...
        # Lines
        for group in lines(self.date_from, self.date_to, self.company_id.id, self.product_id.id, self.parent_categ_id.id, self.categ_id.id, self.invoices_bills):
            # ------------------------------ Partner ------------------------------
            ...
            if self.report_by == 'Summary':
                row += 1
                column = -1
                worksheet.write(row, column+1, group['lines'][0]['partner'])
                worksheet.write(
                    row, column+2, int(group['total_qty']), align_right)
                worksheet.write(
                    row, column+3, int(group['total_amount']), align_right)
            ...
            if self.report_by == 'Detail':
                ...
                worksheet.merge_range(
                    row_number, group['name']['en_US'], cell_text_format)
                ...
                for line in group['lines']:
                    ...
                    worksheet.write(
                        row, column+2, line['invoice_date'], date_format)
                ...
                worksheet.write(row, column+1, 'TOTAL ' +
                                group['name']['en_US'], cell_text_format)
                worksheet.write(row, column+2, '')
                worksheet.write(row, column+3, '')
                worksheet.write(row, column+4, '')
                worksheet.write(
                    row, column+5, int(group['total_qty']), align_right_total)
                worksheet.write(row, column+6, '')
                worksheet.write(
                    row, column+7, int(group['total_amount']), align_right_total)

                total_qty_all += group['total_qty']
                total_amount_all += group['total_amount']
            ...
            worksheet.write(
                row, column+2, int(total_qty_all), align_right_total)
            worksheet.write(
                row, column+3, int(total_amount_all), align_right_total)
            ...
            if self.report_by == 'Detail':
                ...
                worksheet.write(
                    row, column+5, int(total_qty_all), align_right_total)
                worksheet.write(row, column+6, '', align_right_total)
                worksheet.write(
                    row, column+7, int(total_amount_all), align_right_total)
...
class InvoicesbyItemReport(models.AbstractModel):
    ...
    @api.model
    def _lines(self, date_from, date_to, company_id, product_id, parent_categ_id, categ_id, invoices_bills):
        ...
        for key, value in groupby(res, key):
            ...
            for k in value:
                ...
                total_amount += k['amount_total']

            lines.append({'name': key[1], 'lines': sub_lines, 'total_qty': total_qty,
