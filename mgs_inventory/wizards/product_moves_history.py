        ...
        for main in lines(self.product_id.id, self.date_from, self.date_to, self.stock_location_ids.ids, self.partner_id.id, self.include_reserved, self.show_reserved_only, self.order_id.id, 'all'):
            ...
            for product in lines(self.product_id.id, self.date_from, self.date_to, self.stock_location_ids.ids, self.partner_id.id, self.include_reserved, self.show_reserved_only, self.order_id.id, 'yes'):
                ...
                for line in lines(product['product_id'], self.date_from, self.date_to, self.stock_location_ids.ids, self.partner_id.id, self.include_reserved, self.show_reserved_only, self.order_id.id, 'no'):
                    ...
                    qty_in = line['quantity'] if line['location_id_id'] not in self.stock_location_ids.ids else 0
                    qty_out = line['quantity'] if line['location_id_id'] in self.stock_location_ids.ids else 0
                    worksheet.write(
...
class ProductMovesHistoryReport(models.AbstractModel):
    ...
    def _lines(self, product_id, date_from, date_to, stock_location_ids, partner_id, include_reserved, show_reserved_only, order_id, is_group):
        ...
        cases_query = """
        case
            when sld.id in ( """ + ','.join(map(str, stock_location_ids)) + """) then sml.quantity else 0 end as ProductIn,
        case
            when sl.id in (""" + ','.join(map(str, stock_location_ids)) + """) then sml.quantity else 0 end as ProductOut, 0 as Balance
        """
        # +++++++++++++++++++++++
        if is_group == 'all':
            select_query = """select
            COALESCE(sum(case when sld.id in ( """ + ','.join(map(str, stock_location_ids)) + """) then sml.quantity else 0 end), 0) as total_product_in_all,
            COALESCE(sum(case when sl.id in (""" + ','.join(map(str, stock_location_ids)) + """) then sml.quantity else 0 end), 0) as total_product_out_all
            """
        ...
        if is_group == 'yes':
            ...
            COALESCE(sum(case when sld.id in ( """ + ','.join(map(str, stock_location_ids)) + """) then sml.quantity else 0 end), 0) as total_product_in,
            COALESCE(sum(case when sl.id in (""" + ','.join(map(str, stock_location_ids)) + """) then sml.quantity else 0 end), 0) as total_product_out
            """
        ...
        if is_group == 'no':
            select_query = """
            select sml.date, sp.origin,sp.name as picking_id,sml.product_id, 
            COALESCE(case when sml.state = 'done' then sml.quantity else 0 end, 0) as quantity,
            COALESCE(case when sml.state = 'assigned' then sml.quantity else 0 end, 0) as reserved_qty,
            sml.state as state,rp.name as partner_id, pp.id as product_id,
        ...
        and sml.date between %s and %s and pt.type = 'consu' and pt.is_storable = true
        """
