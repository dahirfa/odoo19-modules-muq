...
export class PycusAgeingReportLine extends Component {
    setup(){
        this.action = useService("action");
        this.ormService = useService("orm");
        this.state = useState({
            account_line: this.props.ageing_line,
            ageing_bucket: this.props.ageing_bucket,
            ...
            active_ids: this.props.ageing_line.id_list ? this.props.ageing_line.id_list.slice(0, 2000) : [], // To hold current active ids for pagination
            data: [], //Holds move lines data [{}],
        })

        onMounted(() => {
            this.state.account_line.length = 0
            this.state.account_line = this.props.ageing_line
            this.state.ageing_bucket = this.props.ageing_bucket
        })
        ...
        this.fetchMoveLines = async () => {
            if(this.state.active_ids.length > 0){
                this.state.data = await this.ormService.call(
                    'ins.partner.ageing', 'prepare_detailed_lines',
                    [parseInt(this.state.activeId), this.state.active_ids, this.state.account_line.partner_id]
                    );
            }
        }
...
PycusAgeingReportLine.components = { }
PycusAgeingReportLine.template = 'account_dynamic_reports.ageingReportLine';