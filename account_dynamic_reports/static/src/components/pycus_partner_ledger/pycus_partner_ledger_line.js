...
export class PycusPartnerLedgerLine extends Component {
    setup(){
        ...
        this.fetchMoveLines = async () => {
            if(this.state.active_ids.length > 0){
                this.state.data = await this.ormService.call(
                    'ins.partner.ledger', 'prepare_detailed_lines',
...
PycusPartnerLedgerLine.components = { }
PycusPartnerLedgerLine.template = 'account_dynamic_reports.partnerLedgerLine';