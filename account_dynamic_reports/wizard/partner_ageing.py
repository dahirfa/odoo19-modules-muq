...
class InsPartnerAgeing(models.TransientModel):
    ...
    as_on_date = fields.Date(
        string='As on date', required=True, default=fields.Date.today())
    bucket_1 = fields.Integer(
        string='Bucket 1', required=True, default=lambda self: self.env.company.bucket_1)
    bucket_2 = fields.Integer(
        string='Bucket 2', required=True, default=lambda self: self.env.company.bucket_2)
    bucket_3 = fields.Integer(
        string='Bucket 3', required=True, default=lambda self: self.env.company.bucket_3)
    bucket_4 = fields.Integer(
        string='Bucket 4', required=True, default=lambda self: self.env.company.bucket_4)
    bucket_5 = fields.Integer(
        string='Bucket 5', required=True, default=lambda self: self.env.company.bucket_5)
    include_details = fields.Selection(
        [('yes', 'Yes'), ('no', 'No')], string='Include Details', default='yes')
    ...
    def prepare_bucket_list(self):
        ...
        bucket_list = [self.bucket_1, self.bucket_2,
                       self.bucket_3, self.bucket_4, self.bucket_5]
    ...
    def prepare_main_lines(self):
        ...
        partner_dict['Total'].update(
            {'total': 0.0, 'partner_name': 'ZZZZZZZZZ'})
        partner_dict['Total'].update(
            ...
            for period in period_dict:
                ...
                if not fetch_dict[0].get('balance'):
                    ...
                    amount = fetch_dict[0]['balance'] + \
                        fetch_dict[0]['sum_debit'] - \
                        fetch_dict[0]['sum_credit']
        ...
        total['time_string'] = fields.Datetime.now().strftime(
            "%H:%M:%S") + 'total'
    ...
    def prepare_detailed_lines(self, move_line_ids=[], account_id=False):
        ...
        if account_id:
            ...
            move_lines = []
            for m in final_list:
                m['total'] = m['range_0'] + m['range_1'] + m['range_2'] + \
                    m['range_3'] + m['range_4'] + m['range_5'] + m['range_6']
    ...
    def prepare_values_for_component(self):
        model_id = self.env['ir.model'].sudo().search(
            [('model', '=', 'ins.partner.ageing')], limit=1)

        report_type = {'choices': [], 'selectedValue': {
            'value': self.report_type}}
        partner_type = {'choices': [], 'selectedValue': {
            'value': self.partner_type}}
        include_details = {'choices': [], 'selectedValue': {
            'value': self.include_details}}
    ...
    def action_xlsx(self):
        ...
        # Get record and data
        record = self.env['ins.partner.ageing'].browse(
        ...
        row_pos_2 = 0
        row_pos = 0
        sheet.merge_range(0, 0, 0, 11, 'Partner Ageing' +
        ...
        datestring = fields.Date.from_string(
            str(filter['as_on_date'])).strftime(lang_id.date_format)
        ...
        a_list = ', '.join(
            [lt['code'] or '' for lt in filter.get('defaultPartnerTagsValues')])
        sheet_2.write(row_pos_2, 1, a_list, content_header)
        ...
        for line in ageing_lines:
            ...
            row_pos += 1
            if line.get('id_list'):
                sheet.merge_range(row_pos, 0, row_pos, 3,
                                  line.get('partner_name'), line_header)
            else:
                sheet.merge_range(row_pos, 0, row_pos, 3,
                                  _('Total'), line_header_total)
            k = 4
            for bucket in ageing_bucket:
                sheet.write(
                    row_pos, k, line[ageing_bucket[bucket]['name']], line_header)
                k += 1
            ...
            if record.include_details == 'yes':
                ...
                for sub_line in sub_lines:
                    ...
                    sheet.write(row_pos, 2, sub_line.get(
                        'journal_code'), line_header_light)
                    sheet.write(row_pos, 3, sub_line.get(
                        'account_code') or '', line_header_light)
                    sheet.write(row_pos, 4, float(sub_line.get(
                        'range_0')), line_header_light_period)
                    sheet.write(row_pos, 5, float(sub_line.get(
                        'range_1')), line_header_light_period)
                    sheet.write(row_pos, 6, float(sub_line.get(
                        'range_2')), line_header_light_period)
                    sheet.write(row_pos, 7, float(sub_line.get(
                        'range_3')), line_header_light_period)
                    sheet.write(row_pos, 8, float(sub_line.get(
                        'range_4')), line_header_light_period)
                    sheet.write(row_pos, 9, float(sub_line.get(
                        'range_5')), line_header_light_period)
                    sheet.write(row_pos, 10, float(sub_line.get(
                        'range_6')), line_header_light_period)
                    sheet.write(row_pos, 11, '', line_header_light_period)
        row_pos += 1
        k = 4
        ...
        report_id = self.env['common.xlsx.out'].sudo().create(
            {'filedata': result, 'filename': 'Ageing.xls'})
        return {
