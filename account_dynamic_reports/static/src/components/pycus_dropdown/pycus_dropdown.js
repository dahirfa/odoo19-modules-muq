...
export class PycusDropdown extends Component {
  ...
  setup() {
    ...
    onWillStart(async () => {
      ...
      if (this.props.defaultValues) {
        this.state.selectedValue = {
          value: this.props.defaultValues.map((obj) => obj.value),
        };
        this.props.defaultValues.forEach((item) => {
          this.state.selectedItemsWithLabels.push({
            value: item.value,
            label: item.label,
          });
        });
    ...
    this.fetchOptions = async (term = "") => {
      // Call name_search with positional args: (name, args, operator, limit)
      // and pass context only in kwargs. This avoids sending 'args' as a keyword
      // which causes a TypeError on the server when the method signature doesn't accept it as kw.
      const options = await this.ormService.call(
    ...
    this.handleSelect = async (val) => {
      try {
        if (Array.isArray(val)) {
          // Handle list case
          this.state.selectedItemsWithLabels = val
            .map((value) => {
              const choice = this.state.choices.find(
          ...
          this.state.selectedItemsWithLabels = [{ value: val, label: "aaaaa" }];
